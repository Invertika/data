...
function haitan_talk(npc, ch)
    if (invertika.get_quest_status(ch, "selphi_timlet_haitan_scorpions") == nil) then
        ...
        if v == 1 then
            ...
            npc_message(npc, ch, "Sammle ihre Stachel als Beweis. Solche Skorpione habe ich noch nirgendwo sonst gesehen.")
        else
            npc_message(npc, ch, invertika.get_random_element("Na dann halt nicht...",
    ...
    elseif (invertika.get_quest_status(ch, "selphi_timlet_haitan_scorpions")=="1") then
        ...
        if count < 30 then
            ...
            npc_message(npc, ch, invertika.get_random_element("Worauf wartest du? Los, bring die Skorpione um!"))
        else
            --- Genug umgebracht
            invertika.add_items(ch, 40016, -count, "Kellerskorpionstachel")
            npc_message(npc, ch, "Ich glaube nicht, dass du sie alle umgebracht kriegst. Aber das ist schon mal eine große Hilfe. Hier hast du etwas Schokolade als Belohnung.")
            --- Belohnung ausgeben
            if count > 100 then
        ...
        npc_message(npc, ch, invertika.get_random_element("Vielen Dank für deine Hilfe",
