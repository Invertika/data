...
function PokerSpade:compareSpades(spade1, spade2)
    ...
    elseif self.getSpadeType() < spade2.getSpadeType() then
        ...
        elseif spade1.getSpadeType() == PokerConstants.SPADE_THREE_OF_A_KIND then
            ...
            local rest_cards2 = {}
            for i=1, 5 do
                if self:getNumberOfCardsOfSameValue(cards1, cards1[i]:getValue()) ~= 3
                    table.insert(rest_cards1, cards1[i])
                end
                if self:getNumberOfCardsOfSameValue(cards2, cards2[i]:getValue()) ~= 3
                    table.insert(rest_cards2, cards2[i])
                end
            end
            -- Große Karten an den Anfang
        ...
        elseif spade1.getSpadeType() == PokerConstants.SPADE_FOUR_OF_A_KIND then
            ...
            local rest_cards2 = {}
            for i=1, 5 do
                if self:getNumberOfCardsOfSameValue(cards1, cards1[i]:getValue()) ~= 4
                    table.insert(rest_cards1, cards1[i])
                end
                if self:getNumberOfCardsOfSameValue(cards2, cards2[i]:getValue()) ~= 4
                    table.insert(rest_cards2, cards2[i])
                end
            end
